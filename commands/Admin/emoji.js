...
module.exports.subcommands.add = {
	...
	execute: async (bot, msg, args) => {
		if(!args[0]) return "Please provide a name for the emoji";
		var url;
		if(args[1]) {
		...
		if(!url) return "Please provide a url or attach an image";

		var resp = await bot.fetch(url);
		var img = Buffer.from(await resp.buffer());
		if(img.length > 256000) return "Image is too big, must be below 256kb";
...
module.exports.subcommands.delete = {
	...
	execute: async (bot, msg, args) => {
		if(!args[0]) return "Please provide an emoji to delete";
