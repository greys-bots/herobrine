module.exports = {
	help: ()=> "Create, delete, add, remove, and edit role bundles.",
	...
	execute: (bot, msg, args)=> {
		bot.db.query(`SELECT * FROM bundles WHERE srv_id='${msg.guild.id}'`,(err,rows)=>{
			if(err){
				console.log(err);
				...
				if(rows[0]){
					msg.channel.createMessage({embed: {
						title:"Bundles",
						fields:[{
							name:"\\~\\~\\* Assignable Bundles \\*\\~\\~",
							value: rows.map(b => (b.sa==1 ? b.name.toLowerCase() : null)).filter(x=>x!=null).sort().join("\n") || "None"
						},{
							name:"\\~\\~\\* Mod-Only Bundles \\*\\~\\~",
							value: rows.map(b => (b.sa==0 ? b.name.toLowerCase() : null)).filter(x=>x!=null).sort().join("\n") || "None"
						}]
					}})
				} else {
					msg.channel.createMessage("There are no bundles registered for this server.")
				}
			}
		})
	},
